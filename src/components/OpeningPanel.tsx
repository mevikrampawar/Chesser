...
export function OpeningPanel({ data, analyzing, error, moveCount }: Props) {
  ...
  if (error) {
    return (
      <div className="bg-red-950/50 border border-red-800 rounded-xl p-4 sm:p-6">
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 sm:p-6 space-y-4 sm:space-y-5">
      {analyzing && (
        ...
        <>
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-[10px] sm:text-xs font-mono bg-blue-900/50 text-blue-300 px-2 py-0.5 rounded">
                {data.opening.eco}
              </span>
              {data.lichessOpening && (
                <span className="text-[9px] sm:text-[10px] text-gray-500">Verified by Lichess</span>
              )}
            </div>
            <h2 className="text-lg sm:text-xl font-bold text-white">{data.opening.name}</h2>
          </div>
